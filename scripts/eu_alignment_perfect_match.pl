  ...
  foreach my $mapping (@{$mappings->{'results'}}) {
    ...
    if ($transcript) {
      if ($transcript->translate) {
        ...
        print(UNIPROT_NOSEQS "ENST,$enst_id,could not be translated\n");
      }
    } else {
      print("Transcript $enst_id could not be fetched\n");
      print(UNIPROT_NOSEQS "ENST,$enst_id,could not be fetched from the Ensembl core database using registries $registry_vert and $registry_nonvert\n");
    }
    
    if (!$uniprot_seq) {
      print(UNIPROT_NOSEQS "UNIPROT,$uniprot_id,could not be found in the uniprot files,".join(":",@uniprot_archives)."\n");
    }
    ...
    if ($translation_seq && $uniprot_seq) {
      ...
      if ($score1) {
        print(UNIPROT_SEQS "mapping_id=$mapping_id transcript_id=$gifts_transcript_id enst_id=$enst_id uniprot_id=$uniprot_id STORED as perfect match (sequences found)\n");
      } else {
